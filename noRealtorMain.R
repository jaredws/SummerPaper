...
suppressWarnings("In if (!is.na(offer)) { :
...
house_sales_noRealtor <- as.data.frame(matrix(nrow = 0, ncol = 10))
...
for (t in 1:iter) {
  ...
  SellerList_noRealtor <- append(SellerList_noRealtor, newSellers)
  ...
  for (r in RealtorList_noRealtor) {
    ...
    for (b in BuyerList_noRealtor) {
      ...
      houses <- sample_n(r@Houses, 3, replace = FALSE) %>%
        select(Address, Price, Beds, Baths, SqrFt, LotSize, UpdateTime)
      }
      else{
      ...
      ## else
      ## Let the wait time be commonly within a week
      b@PlayLag = t + ceiling(rexp(1, 1 / 4))
      b <- makeOffer(b)
      if (nrow(b@CurrentOffer) == 1) {
        offerCount  <- offerCount + 1
      }
      ## giveFeedback() is for learning funcitonality and is currently broken
      ## getFeedback() takes on the current offer as well
      #b <- giveFeedback(b)
    ...
    for (s in SellerList_noRealtor) {
      ...
      if (r@Name == s@Realtor) {
        ...
        if (nrow(s@AcceptedOffer) > 0 && !is.na(s@AcceptedOffer)) {
          ...
          BuyerList_noRealtor[length(BuyerList_noRealtor)] <- NULL
